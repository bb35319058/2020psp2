...
- 私のところでこのソースコードをコンパイルしてみたけど，正しい答えになりませんが．．．
- `-DCONST_SEED`をつけてコンパイルしてみた結果が0なのはおかしいです．

[comment #20200722 sonoda]
- 値は何やらでてきますが，`-DCONST_SEED`をつけてコンパイルしてできた実行ファイル`k03`を`k03 170.8 5.43 5`と実行しても上の例のようにはなりません．このときの出力は誰でも同じはずです．（-DCOST_SEEDをつけないときは皆違う結果でいいのですが）
- 入出力結果は，どういうふうに実行したかも書いてください．
